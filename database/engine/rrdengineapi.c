...
void rrdeng_flush_past_metrics_page(RRDDIM_PAST_DATA *dim_past_data, REPLICATION_STATE *rep_state){    
    ...
    info("%s REP: OBSERVE dimension (%s) in time_interval[%lld, %lld] #samples(%d)....END", REPLICATION_MSG, rd->id, (descr->start_time/USEC_PER_SEC), (descr->end_time/USEC_PER_SEC), (descr->page_length/sizeof(storage_number)));
}
